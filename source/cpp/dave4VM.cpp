...
void dave4VM::getDerivativesEdge(std::vector<std::vector<double> > &BxIn,std::vector<std::vector<double> > &ByIn,std::vector<std::vector<double> > &BzIn,double &dx,double &dy){
  Bx=BxIn;
  By=ByIn;
  Bz=BzIn;
  //double C1=2.0/3.0;
...
std::vector<std::vector<point> >  dave4VM::composeMatrixAndInvert(){
  ...
  ******************************************************************/
  ...
  for(int i=0;i<nx;i++){
	...
	double rcond = -1.0;
        int iwork[3*dim*0+11*dim];
	double s[dim];
	lwork = -1;
        dgelsd_(&dim,&dim, &nrhs, amat.data(), &dim, bmat.data(), &dim, s, &rcond, &rank, &wkopt, &lwork,iwork, &info );
        lwork = (int)wkopt;
        work = (double*)malloc( lwork*sizeof(double) );
	dgelsd_(&dim,&dim, &nrhs, amat.data(), &dim, bmat.data(), &dim, s, &rcond, &rank, work, &lwork,iwork, &info );
