...
This script allows you to perform the topology calculations from a specified
starting time, which is an input to this script. It assumes that you are
completing the calculations for the values specified in read_data.txt. 
...
dx = str(360)
dy = str(360)
argTop = inputDir+gap+regionNum+gap+velSmooth+gap+start_dump+gap+ntt+gap+nx+gap+ny+gap+dx+gap+dy+gap+cutoff+gap+sampling+gap+outFile
cwd = os.getcwd()
